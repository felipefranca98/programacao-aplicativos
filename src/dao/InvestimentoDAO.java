...
public class InvestimentoDAO {
	...
	public List<Investimento> buscarInvestimento(Investimento investimento) throws SQLException {
		PreparedStatement st = null;
		ResultSet rs = null;
	...
	public int buscarIdInvestimento(Investimento investimento) throws SQLException {
		...
		try {
			st = conn.prepareStatement("select Id from investimento where Mes = ? and Ano = ? and Investimento = ? and Mensal = ? and Ocasional = ? and Total = ? order by Id");
			st.setInt(1, investimento.getMes());
			st.setInt(2, investimento.getAno());
			st.setString(3, investimento.getInvestimento());
			st.setDouble(4, investimento.getMensal());
